...
[the README's roadmap](../README.md#roadmap); for where to ask for help, see
...
`.torrent` enclosure often _prefers_ the magnet — the two-instance
...
_also_ not private (`[seeding] private = false` — `private` is on by
...
[`docs/SETTINGS.md`](SETTINGS.md#seeding) for both fields' settings-page
documentation.
...
every `v*` tag with notes collected from the `## Release note` section of
each PR merged since the last tag
