...
| `version_semver` — Semantic Versioning or Calendar Versioning is used. | SUGGESTED | Met | `[workspace.package].version` in Cargo.toml follows semver; the release job fails if a tag disagrees with it. [Cargo.toml](https://github.com/ivylikethevine/sharerr-rs/blob/main/Cargo.toml). |
