...
fn parse(raw: &[u8]) -> Result<([u8; SALT_LEN], BTreeMap<String, Record>)> {
    ...
    // Zero-filled only as a buffer to decode into; overwritten on the next line
    // with the salt actually stored in this vault file, which was random when it
    // was generated. Nothing here is a fixed cryptographic value.
